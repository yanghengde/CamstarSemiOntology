# HistoryMainline → HAS_BATCH → Container
...
- JOIN 条件：`src.[BatchId] = tgt.[ContainerId]`
- 物理外键：`[HistoryMainline].[BatchId]`
...
LEFT JOIN [Container] AS tgt
    ON src.[BatchId] = tgt.[ContainerId]
...
- 本体关系：`HistoryMainline --[HAS_BATCH]--> Container`
- 基数：`MANY_TO_ONE`
- 物理定义：`HistoryMainline.BatchId`
