# MaintenanceReq → HAS_BOM → A_ResourceBOM
...
- JOIN 条件：`src.[BOMId] = tgt.[ResourceBOMId]`
- 物理外键：`[MaintenanceReq].[BOMId]`
...
LEFT JOIN [A_ResourceBOM] AS tgt
    ON src.[BOMId] = tgt.[ResourceBOMId]
...
- 本体关系：`MaintenanceReq --[HAS_BOM]--> A_ResourceBOM`
- 基数：`MANY_TO_ONE`
- 物理定义：`MaintenanceReq.BOMId`
